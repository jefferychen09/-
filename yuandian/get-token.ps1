# get-token.ps1 — Fetch Yuandian API Key from credential gateway
#
# Usage:
#   $apiKey = & .\get-token.ps1
#
# Token is auto-injected via local proxy service, no manual input needed
...
$Platform  = if ($env:CREDENTIAL_PLATFORM) { $env:CREDENTIAL_PLATFORM } else { "yuandian" }
...
try {
    ...
    [Console]::Error.WriteLine("ERROR: Gateway request failed: $_. Please authorize Yuandian in the integration panel first.")
    exit 1
}

if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: Gateway returned error (ret=$($response.ret)). Please authorize Yuandian in the integration panel first.")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: API Key not found. Please authorize Yuandian in the integration panel first.")
    exit 1
}
