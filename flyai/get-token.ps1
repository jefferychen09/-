# get-token.ps1 — Retrieve FlyAI access_token from credential proxy
#
# Usage:
#   $token = & .\get-token.ps1
...
# BUILD_ENV=test uses test env; otherwise (including unset) uses production
...
if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: ret=$($response.ret), please complete FlyAI authorization in the integration panel first")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: failed to retrieve access_token, please complete FlyAI authorization in the integration panel first")
    exit 1
}
