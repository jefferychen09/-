...
完整的数据结构和接口参数详见 `<SKILL_DIR>/knowledge-base/references/api.md`。
...
使用 `<SKILL_DIR>/knowledge-base/scripts/preflight-check.cjs` 脚本自动完成类型检测和大小校验。脚本按以下优先级解析：
...
完整字段定义见 `<SKILL_DIR>/knowledge-base/references/api.md`。
