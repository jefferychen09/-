# get-token.ps1 — Retrieve Youdao Note API Key from credential proxy
#
# Usage:
...
if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: ret=$($response.ret), please complete Youdao Note authorization in the integration panel first")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: failed to retrieve API Key, please complete Youdao Note authorization in the integration panel first")
    exit 1
}
