# get-token.ps1 — Retrieve Wendao API Key from credential proxy
#
# Usage:
#   $token = & .\get-token.ps1
...
if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: ret=$($response.ret), please complete Wendao authorization in the integration panel first")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: failed to retrieve access_token, please complete Wendao authorization in the integration panel first")
    exit 1
}
