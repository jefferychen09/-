# get-token.ps1 — Retrieve Notion access_token from credential proxy
#
# Usage:
#   $token = & .\get-token.ps1             # capture to variable
#   $env:NOTION_TOKEN = & .\get-token.ps1  # assign to env var
#
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: failed to retrieve access_token, please complete Notion authorization in the integration panel first")
    exit 1
}
