# get-token.ps1 — Fetch QCC API Key from credential gateway
#
# Usage:
...
$Platform  = if ($env:CREDENTIAL_PLATFORM) { $env:CREDENTIAL_PLATFORM } else { "qcc" }
...
try {
    ...
    [Console]::Error.WriteLine("ERROR: Gateway request failed: $_. Please authorize QCC in the integration panel first.")
    exit 1
}

if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: Gateway returned error (ret=$($response.ret)). Please authorize QCC in the integration panel first.")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: API Key not found. Please authorize QCC in the integration panel first.")
    exit 1
}
