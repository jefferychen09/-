...
case "$(uname -s 2>/dev/null)" in
    MINGW*|MSYS*|CYGWIN*) _WJ_PLATFORM="windows_bash" ;;
esac
...
# 临时文件：跨平台适配目录路径，避免 symlink 攻击和多用户冲突
if [[ "$_WJ_PLATFORM" == "windows_bash" ]]; then
    # Windows Git Bash / MSYS2: use TEMP or USERPROFILE, id -u may not exist
    _WJ_TMP_BASE="${TEMP:-${USERPROFILE}/AppData/Local/Temp}"
    _WJ_TMP_DIR="${_WJ_TMP_BASE}/.wj_auth_${USERNAME:-$(whoami 2>/dev/null || echo default)}"
else
...
_wj_check_dependencies() {
    local missing=()
    # On Windows Git Bash, prefer curl.exe to avoid shell alias conflicts
    if [[ "$_WJ_PLATFORM" == "windows_bash" ]]; then
        command -v curl.exe &>/dev/null || command -v curl &>/dev/null || missing+=("curl")
    else
    ...
    if [[ -n "$1" ]]; then
        ...
        case "$1" in
            ...
            *)
                ...
                echo ""
                echo "Windows users: use setup.ps1 instead"
                echo "  powershell -ExecutionPolicy Bypass -File setup.ps1 wj_check_and_start_auth"
                exit 1
                ;;
        esac
    else
        ...
        echo ""
        if [[ "$_WJ_PLATFORM" == "windows_bash" ]]; then
            echo "Note: You are running in Windows Git Bash."
            echo "For native Windows, consider using setup.ps1 instead:"
            echo "  powershell -ExecutionPolicy Bypass -File .\\setup.ps1 wj_check_and_start_auth"
        fi
    fi
fi
