...
├── setup.sh              # 自动化配置与授权脚本（macOS/Linux/Git Bash）
├── setup.ps1             # 自动化配置与授权脚本（Windows PowerShell）
├── references/           # 工具参考文档
│   ├── auth.md           # 鉴权流程说明
...
- **Windows 用户**：需要 PowerShell 5.1+（系统自带）和 `curl.exe`（Windows 10+ 自带）
...
| 工具 | 说明 | 参考文档 |
|------|------|---------|
| `get_survey` | 获取问卷详情（标题、页面、题目、选项 + DSL + 自定义逻辑） | [get_survey.md](references/get_survey.md) |
...
| `update_logic` | 更新问卷的自定义逻辑设置（条件显示/隐藏、跳转、替换、随机等） | [update_logic.md](references/update_logic.md) |
...
# 设置条件显示逻辑（需先通过 get_survey 获取题目/选项 ID）
mcporter call tencent-survey.update_logic --args '{"survey_id": 12345, "dsl": "if `q-1-abcd::o-100-EFGH` then show `q-1-ijkl`"}'

# 清空所有自定义逻辑
mcporter call tencent-survey.update_logic --args '{"survey_id": 12345, "dsl": ""}'
...
```bash
# Step 1: Check status (returns immediately)
bash ./setup.sh wj_check_and_start_auth
# Output READY → proceed
# Output AUTH_REQUIRED:<url> → show link to user, then run step 2

# Step 2: Wait for authorization (only when AUTH_REQUIRED)
bash ./setup.sh wj_wait_auth
# Output TOKEN_READY:* → auth success
```
