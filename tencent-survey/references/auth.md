...
**Windows (PowerShell)：**
```powershell
powershell -ExecutionPolicy Bypass -File "${SKILL_DIR}\setup.ps1" wj_check_and_start_auth
```

> `${SKILL_DIR}` 为当前 skill 所在目录路径（即 `setup.sh` / `setup.ps1` 所在目录）。
...
**Windows (PowerShell)：**
```powershell
powershell -ExecutionPolicy Bypass -File "${SKILL_DIR}\setup.ps1" wj_wait_auth
```
...
**Windows (PowerShell)：**
```powershell
$env:TENCENT_SURVEY_TOKEN="<your_token>"; powershell -ExecutionPolicy Bypass -File "${SKILL_DIR}\setup.ps1" wj_check_and_start_auth
```

或手动执行 mcporter 命令（所有平台通用）：
