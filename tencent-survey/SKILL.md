...
version: 1.1.1
metadata: {"openclaw":{"category":"tencent","tencentTokenMode":"custom","emoji":"📋"}}
---
...
**在本次会话首次调用工具前**，必须先执行鉴权检查（完整流程见 `references/auth.md`）：

> `${SKILL_DIR}` 为本 skill 所在目录路径（即 `SKILL.md` 所在目录）。由 AI Agent 框架在加载 skill 时自动注入；如果框架未注入，请替换为 `SKILL.md` 所在目录的绝对路径。

### 鉴权判断规则（重要）

> ⚠️ **必须通过执行 `wj_check_and_start_auth` 脚本来判断是否已授权。**
>
> - 脚本会检查 mcporter 配置中是否已存在 `tencent-survey` 服务及有效 Token。
> - **不能**仅凭环境变量 `TENCENT_SURVEY_TOKEN` 是否存在来判断授权状态。环境变量仅作为"写入配置"的输入源，不代表已完成配置。
> - 只有脚本输出 `READY` 才表示鉴权就绪。
...
与 macOS / Linux 相同命令，`setup.sh` 已内置 MSYS2/Git Bash 兼容。

### 环境变量传入 Token（可选）

如果用户已有 Token，可通过环境变量传入让脚本写入 mcporter 配置：
...
> 注意：环境变量仅在 mcporter 配置中无有效 Token 时才会被使用。脚本始终优先检查 mcporter 已有配置。
...
| `paid_function_trial_no_permission` | 付费功能无权限（如自定义逻辑） | 引导用户前往[腾讯问卷购买页](https://wj.qq.com/pricing)升级为高级版/专业版 |
