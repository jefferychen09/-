#!/usr/bin/env bash
# get-token.sh — 从凭证托管服务获取 Figma access_token
#
# 用法:
...
# 远程 API 基础地址（根据 BUILD_ENV 区分环境，与 skill-interceptor 一致）
if [[ "${BUILD_ENV:-test}" == "production" ]]; then
...
PLATFORM="${CREDENTIAL_PLATFORM:-figma}"
...
if [[ -z "$access_token" || "$access_token" == "null" ]]; then
  echo "ERROR: 未获取到 access_token，请先在集成面板中完成 Figma 授权" >&2
  exit 1
fi
