...
> **User-facing 边界：** 本文是 CLI/agent 内部执行参考，不是最终用户回复模板。最终回复不得把任何 xb CLI 命令直接当作自然语言流程步骤输出，包括状态、白名单、日志、引导、放行、撤销、运行、版本等命令。涉及网络防护时，用自然语言说明识别原因、风险、明确确认和“由 agent 内部执行”；需要排查最近拦截原因时，说“我会在内部查询最近的防护记录/拦截记录”，不要复制具体 CLI 命令；加白或继续访问场景不得提到撤销白名单命令，除非用户要求撤销已有白名单或清理测试状态。
...
```bash
xb guide <config|close-browser|incomplete-config|shield-allow|shield-off>
```

用于引导用户完成配置或确认操作，返回面向用户的提示文案。

> 注意：本节是 CLI/agent 执行参考，不是最终用户回复模板。自然语言回复中应表达为确认流程，并等待用户明确选择；不能把 `user_choice_mapping` 中的命令当成用户已经确认。
...
### 3.4 guide shield-allow — 加白名单引导

```bash
xb guide shield-allow <host:port>
```

当网络防护拦截可放行的 `host:port`，且用户明确希望放行时使用。返回决策点，agent **必须**按 SKILL.md"处理决策点返回"章节处理。

> 注意：本节是 CLI/agent 执行参考，不是最终用户回复模板。自然语言回复中应表达为确认流程；`guide` 只是确认引导，不代表操作已完成；`remove` 只用于撤销白名单，不能作为加白方式；`enable` 只用于恢复防护，不能作为关闭方式。
...
```json
{
  ...
  "data": {
    ...
    "options": [
      { "value": "cancel", "label": "取消（推荐）", "description": "保持拦截，不修改白名单" },
      { "value": "confirm", "label": "确认加入白名单", "description": "允许访问该 host:port" }
    ],
    "recommended": "cancel",
    "user_choice_mapping": {
      "cancel": null,
...
> agent 不应推断、构造或补全任何后续命令。需要用户确认的操作，必须先执行对应的 `xb guide ...` 命令；只有当前 guide 返回的 `user_choice_mapping` 中明确给出的命令，才允许在用户选择后执行。

### 3.5 guide shield-off — 关闭防护引导

```bash
xb guide shield-off
```

仅当用户明确要求关闭整个网络防护时使用。强烈不推荐关闭防护；如果只是访问特定地址，优先使用 `xb guide shield-allow <host:port>`。

返回决策点，agent **必须**展示风险说明并等待用户选择，不得代用户确认。
...
```json
{
  ...
  "error": "缺少子命令",
  "hint": "xb help guide",
  "data": { "subcommands": ["config", "close-browser", "incomplete-config", "shield-allow", "shield-off"] }
}
```
...
```bash
xb shield <status|list|logs|enable|remove>
```

网络防护默认启用，用于拦截内网地址、云元数据端点、危险协议等访问。详细安全模型见 [xb-shield.md](./xb-shield.md)。

> agent 不应根据 `status`、`list`、`logs`、`enable`、`remove` 等公开命令推断、构造或补全其它操作命令。白名单和关闭防护这类需要用户确认的操作，唯一合法来源是 `xb guide shield-allow` / `xb guide shield-off` 当前返回的 `user_choice_mapping`。
>
> 语义边界：`status`/`list`/`logs` 只用于内部查询，不是用户流程步骤；`remove` 只撤销白名单，不能作为加白或放行方式；`enable` 只恢复防护，不能作为关闭方式；guide 返回只是确认流程，不代表操作完成。
>
> 用户可见边界：安全流程、注意事项、拦截排查类最终回复不得复制本节任何 raw 命令；排查最近拦截原因时，用“内部查询最近的防护记录/拦截记录”表达。

### 8.1 shield status — 查看防护状态

```bash
xb shield status
```

返回防护启用状态、白名单数量和 24 小时拦截次数。

### 8.2 shield list — 查看白名单
...
### 8.3 shield logs — 查看防护日志

```bash
xb shield logs [--limit N]
```

返回最近防护日志，默认 20 条。`--limit` 支持 1-1000。向用户说明排查动作时只用自然语言，不复制本命令。

### 8.4 shield enable — 启用防护
...
### 8.5 shield remove — 删除白名单条目

```bash
xb shield remove <host:port>
```

删除一个白名单条目。仅删除确认由用户授权或测试新增的条目，不要删除用户原有配置。
...
| `guide shield-allow` | 加白名单引导 | `xb guide shield-allow <host:port>` |
| `guide shield-off` | 关闭防护引导 | `xb guide shield-off` |
| `setup` | 安装引擎和浏览器 | `xb setup` |
| `stop` | 关闭本地浏览器 | `xb stop <id\|all> [--force]` |
| `cleanup` | 清理会话 | `xb cleanup` |
| `status` | 环境状态 | `xb status` |
| `shield status` | 网络防护状态 | `xb shield status` |
| `shield list` | 白名单列表 | `xb shield list` |
| `shield logs` | 防护日志 | `xb shield logs [--limit N]` |
| `shield enable` | 启用防护 | `xb shield enable` |
| `shield remove` | 删除白名单 | `xb shield remove <host:port>` |
